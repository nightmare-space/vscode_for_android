...
command_login() {
	...
	if [ "$target_arch" = "unknown" ]; then
		if [ -f "${DISTRO_PLUGINS_DIR}/${distro_name}.sh" ]; then
			# shellcheck disable=SC1090
			target_arch=$(
				. "${DISTRO_PLUGINS_DIR}/${distro_name}.sh"
				echo "${DISTRO_ARCH}"
			)
		elif [ -f "${DISTRO_PLUGINS_DIR}/${distro_name}.override.sh" ]; then
			# shellcheck disable=SC1090
			target_arch=$(
				. "${DISTRO_PLUGINS_DIR}/${distro_name}.override.sh"
				echo "${DISTRO_ARCH}"
			)
		else
	...
	echo isolated_environment: $isolated_environment
	echo need_cpu_emulator: $need_cpu_emulator
		...
		for i in $(echo "${!SUPPORTED_DISTRIBUTIONS[@]}" | tr ' ' '\n' | sort -d); do
			if $verbose; then
				...
				if [ -f "${DISTRO_PLUGINS_DIR}/${i}.sh" ]; then
					supported_cpus=$(
						source "${DISTRO_PLUGINS_DIR}/${i}.sh"
						echo "${!TARBALL_URL[@]}"
					)
				elif [ -f "${DISTRO_PLUGINS_DIR}/${i}.override.sh" ]; then
					supported_cpus=$(
						source "${DISTRO_PLUGINS_DIR}/${i}.override.sh"
						echo "${!TARBALL_URL[@]}"
					)
				else
...
if [ $# -ge 1 ]; then
	...
	remove | rm)
		shift 1
		CMD_REMOVE_REQUESTED_RESET="false" command_remove "$@"
		;;
	rename | mv)
		shift 1
		command_rename "$@"
